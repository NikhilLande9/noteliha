...
class SyncState {
  ...
  // Tracks which Google account last synced on this device.
  // Used to detect account switches so the Drive check is re-run
...
// ─────────────────────────────────────────────────────────────────────────────
// NEW: Enum to represent the Drive account state after sign-in check
// ─────────────────────────────────────────────────────────────────────────────
...
class NotesProvider extends ChangeNotifier {
  ...
  Future<void> init() async {
    try {
      ...
      _googleSignIn.onCurrentUserChanged.listen((account) async {
        ...
        if (account != null) {
          // Skip the Drive check only if this exact account has previously
          // synced on this device (manifestFileId present AND email matches).
          // If a different account signs in, the stored sync state belongs to
          // someone else — we must re-check Drive for the new account.
      ...
      // If silent sign-in succeeded, _checkDriveManifestExists is triggered
      // via the listener above. If it returned null, just notify.
  ...
  // ── NEW: Check if manifest exists on Drive ────────────────────────────────
  /// Checks Drive for an existing manifest and updates [driveAccountState].
  /// Called automatically after sign-in. Can also be called manually.
  Future<void> _checkDriveManifestExists() async {
    ...
    try {
      ...
      // Look for the root app folder first
      ...
      if (rootList.files?.isEmpty ?? true) {
        // No root folder → definitely a new user
      ...
      // Root folder found — now check for manifest.json inside it
      ...
      if (manifestList.files?.isNotEmpty ?? false) {
        ...
        // Root folder exists but no manifest (incomplete backup or first run)
        _syncState.rootFolderId = rootId;
        _persistSyncState();
        _driveAccountState = DriveAccountState.newUser;
        _setStatus('No backup found. Upload to create first backup.',
      ...
      // On error fall back to unknown so both buttons remain usable
      _driveAccountState = DriveAccountState.unknown;
  ...
  Future<void> signIn() async {
    try {
      ...
      // _checkDriveManifestExists is triggered by onCurrentUserChanged listener
      notifyListeners();
    } catch (e) {
  ...
  Future<void> uploadNotes() async {
    ...
    try {
      ...
      // After first successful upload, mark as returning user
      _driveAccountState = DriveAccountState.returningUser;
...
// ─────────────────────────────────────────────────────────────────────────────
// Settings Screen — updated to reflect Drive account state
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// _LoggedInTile — drive-aware upload / sync buttons
// ─────────────────────────────────────────────────────────────────────────────
class _LoggedInTile extends StatelessWidget {
  ...
  /// True while we are still probing Drive after sign-in.
  bool get _isChecking =>
      prov.isCheckingDrive ||
      prov.driveAccountState == DriveAccountState.unknown;

  /// Whether a backup was found on Drive (returning user on a new device).
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        // ── Drive state indicator while checking ───────────────────────────
        if (_isChecking)
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            child: Row(
        ...
        // ── Upload Backup button (always shown, disabled while checking) ────
        ListTile(
        ...
        // ── Restore from Drive button (disabled for new users) ─────────────
        ListTile(
          leading: Icon(
          ...
          // Disabled for new users who have no remote backup yet
