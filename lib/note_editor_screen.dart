...
// Sort modes for the checklist toolbar
enum _ChecklistSortMode {
  none,
  alphabetical,
  alphabeticalDesc,
  pendingFirst,
...
class _NoteEditorScreenState extends State<NoteEditorScreen>
  ...
  // base64Decode is expensive. Cache decoded Uint8List per imageId so the
  // image strip never decodes on the main thread during a rebuild.
  final Map<String, Uint8List> _decodedThumbnails = {};
  ...
  _ChecklistSortMode _checklistSortMode = _ChecklistSortMode.none;
  ...
  bool hasUnsavedChanges() {
    ...
    if (savedRecipe == null && editRecipe != null) {
      ...
      if (editRecipe.ingredientRows.length !=
          savedRecipe.ingredientRows.length) {
        return true;
      }
  ...
  Future<void> _pickImage() async {
    try {
      ...
      final images = await _picker.pickMultiImage(
        imageQuality: 75, // compress to ~75% quality (good visual/size balance)
        maxWidth: 1920, // cap at 1080p-wide — plenty for mobile display
        maxHeight: 1920,
        limit: 5, // allow up to 5 images at once
      );
      if (images.isNotEmpty && mounted) {
        ...
        setState(() => _editing.imageIds.addAll(newIds));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${context.tr("could_not_pick_image")}: $e'),
...
// value has been entered.
  ...
  void _save() {
    ...
    // For drawing notes, content is already updated by TouchWritingCanvas.onChanged.
    // Don't overwrite it with rich text serialization.
    if (_editing.noteType != NoteType.drawing) {
    ...
    if (!_hasContent()) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(context.tr('nothing_to_save')),
          behavior: SnackBarBehavior.floating,
          shape: const RoundedRectangleBorder(
              borderRadius: BorderRadius.all(Radius.circular(10))),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final newLabel = widget.note == null
        ? AppTranslations.translate('new')
        : AppTranslations.translate('edit');
    final editorTitle = '$newLabel ${_noteTypeLabel(_editing.noteType)}';
          ...
          Row(
            children: [
              ...
              if (widget.note != null &&
                ...
                Tooltip(
                  message: context.tr('encrypted_label'),
                  child: Padding(
                    padding: const EdgeInsets.only(right: 6),
      ...
      child: SizedBox(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            // Use listen:false — the strip rebuilds via setState when imageIds
            // changes, not via provider notifications on every frame.
            final prov = Provider.of<NotesProvider>(context, listen: false);

            // Decode once and cache; base64Decode on the main thread every
            // rebuild was the primary cause of the 5-image slowdown.
            if (!_decodedThumbnails.containsKey(imgId)) {
              final b64 = prov.getImage(imgId);
              if (b64 != null) {
                try {
                  _decodedThumbnails[imgId] = base64Decode(b64);
                } catch (_) {}
              }
            }
            final bytes = _decodedThumbnails[imgId];
            ...
            return Container(
              ...
              child: NeuContainer(
                ...
                child: ClipRRect(
                  ...
                  child: Stack(
                    children: [
                      // ── Thumbnail (tappable → full-screen viewer) ─────────
                      GestureDetector(
                        onTap: () {
                          ImageViewer.show(
                            context,
                        ...
                        child: GestureDetector(
                          onTap: () async {
                            ...
                            setState(() {
                              ...
                              _decodedThumbnails.remove(imageId);
                            });
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(context.tr('delete_note_to_bin'),
                  style: TextStyle(
        ...
        Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  // ── Sort menu (single icon → popup list) ──────────────────
                  _ChecklistSortMenuButton(
                    isDark: isDark,
                    accent: accent,
                    sortMode: _checklistSortMode,
                    onSelected: (mode) => setState(() {
                      // Tapping the active mode deselects it.
                      if (_checklistSortMode == mode) {
                        _checklistSortMode = _ChecklistSortMode.none;
                        return;
                      }
                      _checklistSortMode = mode;
                      switch (mode) {
                        case _ChecklistSortMode.alphabetical:
                          _editing.checklistItems
                              .sort((a, b) => a.text.compareTo(b.text));
                        case _ChecklistSortMode.alphabeticalDesc:
                          _editing.checklistItems
                              .sort((a, b) => b.text.compareTo(a.text));
                        case _ChecklistSortMode.pendingFirst:
                          _editing.checklistItems.sort((a, b) {
                            if (a.checked != b.checked) {
                              return a.checked ? 1 : -1;
                            }
                            return a.text.compareTo(b.text);
                          });
                        case _ChecklistSortMode.completedFirst:
                          _editing.checklistItems.sort((a, b) {
                            if (a.checked != b.checked) {
                              return a.checked ? -1 : 1;
                            }
                  ...
                  _ChecklistToolbarButton(
                    label: _checklistCompact
                        ? context.tr('expand_items')
                        : AppTranslations.translate('compact_view'),
  ...
  void toggleInline(_FormatType type, TextSelection sel) {
    ...
    if (covering.isNotEmpty) {
      for (final r in covering) {
        ...
        if (r.end > e) {
          _ranges.add(_FmtRange(start: e, end: r.end, type: type));
        }
      }
    } else {
...
class _RichTextFieldState extends State<_RichTextField> {
  ...
  void _scrollToMatch(int charOffset) {
    ...
    final target = (estimatedY - viewportHeight / 2).clamp(0.0, maxScroll);
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: context.tr('start_writing'),
          hintStyle: TextStyle(
            fontSize: 15.5,
            height: 1.6,
            color: Neu.textTertiary(isDark),
          ...
          Row(
            children: [
              ...
              AnimatedDefaultTextStyle(
                ...
                child: Text(
                  total == 0
                      ? AppTranslations.translate('no_items_yet')
                      : '$done / $total ${AppTranslations.translate("completed")}',
                ),
              ),
              if (allDone) ...[
                const SizedBox(width: 6),
...
// ─────────────────────────────────────────────────────────────────────────────
// Checklist sort icon → popup menu
// ─────────────────────────────────────────────────────────────────────────────

class _ChecklistSortMenuButton extends StatelessWidget {
  final bool isDark;
  final Color accent;
  final _ChecklistSortMode sortMode;
  final ValueChanged<_ChecklistSortMode> onSelected;

  const _ChecklistSortMenuButton({
    required this.isDark,
    required this.accent,
  ...
  @override
  Widget build(BuildContext context) {
    final isActive = sortMode != _ChecklistSortMode.none;
    return PopupMenuButton<_ChecklistSortMode>(
      tooltip: '',
      ...
      itemBuilder: (_) => [
        _sortMenuItem(
          context,
          value: _ChecklistSortMode.alphabetical,
          icon: Icons.sort_by_alpha_rounded,
          label: 'A→Z',
        ),
        _sortMenuItem(
          context,
          value: _ChecklistSortMode.alphabeticalDesc,
          icon: Icons.sort_by_alpha_rounded,
          label: 'Z→A',
        ),
        _sortMenuItem(
          context,
          value: _ChecklistSortMode.pendingFirst,
          icon: Icons.radio_button_unchecked_rounded,
          label: AppTranslations.translate('pending_first'),
        ),
        _sortMenuItem(
          context,
          value: _ChecklistSortMode.completedFirst,
          icon: Icons.check_circle_outline_rounded,
          label: AppTranslations.translate('completed_first'),
        ),
      ],
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 150),
      ...
      child: Row(
        children: [
          Icon(
            icon,
            size: 16,
            color: isSelected ? accent : Neu.textSecondary(isDark),
          ),
          const SizedBox(width: 10),
          Text(
            label,
            style: TextStyle(
              ...
              color: isSelected ? accent : Neu.textPrimary(isDark),
            ),
          ),
          if (isSelected) ...[
            const Spacer(),
      ...
      itemBuilder: (_) => [
        PopupMenuItem(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            Text(editing.pinned
                ? AppTranslations.translate('unpin')
                : AppTranslations.translate('pin')),
          ]),
        ),
        PopupMenuItem(
          value: 'add_image',
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            Text(AppTranslations.translate('add_image')),
          ]),
        ),
          ...
          PopupMenuItem(
            ...
            child: Row(children: [
              Icon(checklistCompact
                  ...
                  : Icons.unfold_less_rounded),
              const SizedBox(width: 10),
              Text(checklistCompact
                  ? AppTranslations.translate('expand_items')
                  : AppTranslations.translate('compact_view')),
            ]),
          ),
        ...
        PopupMenuItem(
          enabled: false,
          height: 28,
          child: Text(AppTranslations.translate('color_theme'),
        ...
        if (onDelete != null) ...[
          ...
          PopupMenuItem(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 10),
              Text(AppTranslations.translate('delete'),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _SectionLabel(
              text: AppTranslations.translate('ingredients').toUpperCase(),
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(
                  flex: 4,
                  child: Text(AppTranslations.translate('ingredient_name'),
                      style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                          color: ter,
              ...
              Expanded(
                  flex: 2,
                  child: Text(AppTranslations.translate('quantity'),
                      style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                          color: ter,
                    ...
                    if (isActive && row.name.isNotEmpty) ...[
                      ...
                      Padding(
                        ...
                        child: _HighlightedText(
                          ...
                          query: widget.searchQuery,
                          baseStyle: TextStyle(
                              fontSize: 11.5, color: Neu.textSecondary(isDark)),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                        ),
                      ),
          ...
          NeuPressable(
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 6),
                Text(AppTranslations.translate('add_ingredient'),
                    style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w600,
                        color: accent)),
          ...
          const SizedBox(height: 24),
          _SectionLabel(
              text: AppTranslations.translate('steps').toUpperCase(),
              icon: Icons.format_list_numbered_rounded,
              isDark: isDark),
          const SizedBox(height: 8),
          ...
          NeuPressable(
            ...
            child: Row(
              ...
              children: [
                Icon(Icons.add_rounded, size: 16, color: accent),
                const SizedBox(width: 6),
                Text(AppTranslations.translate('add_step'),
                    style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w600,
                        color: accent)),
