...
class NoteCard extends StatelessWidget {
  final Note note;
  final String decryptedContent; // Added for decrypted content
  final String searchQuery; // Highlights matching text in the card preview
  ...
  Widget _buildNormalPreview(Color subColor) {
    // If the content couldn't be decrypted, show a warning instead of
    // attempting to decode ciphertext as rich text (which would crash or
    // produce garbage output).
    if (decryptedContent == NotesProvider.kUndecryptablePlaceholder) {
      return Row(
        children: [
          ...
          Expanded(
            child: Text(
              AppTranslations.translate('encrypted_different_key'),
              maxLines: 2,
              overflow: TextOverflow.ellipsis,
              style: TextStyle(
                  fontSize: 12, color: Colors.orange.shade600, height: 1.4),
            ),
          ),
        ],
      );
    ...
    // Use decrypted content instead of raw note.content
    final decoded = RichContentCodec.decode(decryptedContent);
  ...
  Widget build(BuildContext context) {
    Provider.of<LanguageProvider>(context); // rebuild when language changes
        ...
        child: InkWell(
          ...
          child: Stack(
            children: [
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        Expanded(
                          child: Text(
                            note.title.isNotEmpty
                                ? note.title
                                : AppTranslations.translate('untitled'),
                            style: TextStyle(
                              fontSize: 14,
                    ...
                    // Normal notes: rich preview with bold/italic using decrypted content.
                    // Drawing notes: mini canvas thumbnail.
                    // All other types: plain text string, with search highlights.
                    if (note.noteType == NoteType.normal) ...[
                      const SizedBox(height: 6),
                      _buildNormalPreview(subColor),
                    ] else if (note.noteType == NoteType.drawing) ...[
                      const SizedBox(height: 6),
                      _buildDrawingPreview(isDark, accentColor),
                    ...
                    if (note.imageIds.isNotEmpty) ...[
                      ...
                      Row(children: [
                        ...
                        Text(
                          '${note.imageIds.length} ${note.imageIds.length == 1 ? AppTranslations.translate("image") : AppTranslations.translate("images")}',
                    ...
                    Row(
                      children: [
                        ...
                        _NeuCategoryChip(
                            note: note, isDark: isDark, accent: accentColor),
                        const Spacer(),
                        // Encryption badge — shown when stored content is ciphered
                        if (note.content.startsWith('ENC1:')) ...[
                        ...
                        // Eye icon — always visible, taps open full preview
                        // regardless of selection mode. HitTestBehavior.opaque
                        // ensures the gesture is captured here and does not
                        // bubble up to the card's InkWell / selection handler.
                        GestureDetector(
              ...
              // Selection tick overlay in top-right corner
              if (isSelected)
                Positioned(
                  top: 10,
                  right: 10,
  ...
  /// Renders a small read-only canvas thumbnail for drawing notes.
  Widget _buildDrawingPreview(bool isDark, Color accent) {
    if (decryptedContent == NotesProvider.kUndecryptablePlaceholder) {
      return const Row(children: [
        Icon(Icons.lock_rounded, size: 13, color: Colors.orange),
        SizedBox(width: 5),
        Text('🔒 Encrypted',
    ...
    try {
      final strokes = strokesFromJson(decryptedContent);
      if (strokes.isEmpty) {
        return Row(children: [
          Icon(Icons.brush_rounded, size: 13, color: accent.withAlpha(160)),
          const SizedBox(width: 5),
          Text(AppTranslations.translate('drawing'),
      ...
      // Malformed JSON — fall back to label
      return Row(children: [
        Icon(Icons.brush_rounded, size: 13, color: accent.withAlpha(160)),
        const SizedBox(width: 5),
        Text(AppTranslations.translate('drawing'),
  ...
  String _formatDate(DateTime dt) {
    ...
    if (diff.inMinutes < 1) {
      return AppTranslations.translate('just_now');
    }
    if (diff.inHours < 1) {
      return '${diff.inMinutes}${AppTranslations.translate('min_ago')}';
    }
    if (diff.inDays < 1) {
      return '${diff.inHours}${AppTranslations.translate('hr_ago')}';
    }
    if (diff.inDays == 1) {
      return AppTranslations.translate('yesterday');
    }
    if (diff.inDays < 7) {
...
class _NoteListScreenState extends State<NoteListScreen> {
  ...
  // Tracks which category sections are expanded in category-sort mode.
  // Categories are collapsed by default; tapping the header toggles them.
  ...
  void _selectAll(List<Note> allVisible) {
    setState(() {
      _selectedNoteIds.addAll(allVisible.map((n) => n.id));
    });
  }

  Future<void> _deleteSelected(BuildContext context, NotesProvider prov) async {
    final count = _selectedNoteIds.length;
    final isDark = Theme.of(context).brightness == Brightness.dark;

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => _NeuAlertDialog(
        isDark: isDark,
        message: AppTranslations.translate('delete_selected_confirmation')
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            AppTranslations.translate('notes_deleted')
                .replaceAll('{count}', '$count'),
          ),
    ...
    for (int i = 0; i < notes.length; i++) {
      ...
      switch (note.noteType) {
        ...
        case NoteType.drawing:
          final decrypted = prov.getDecryptedContent(note);
          if (decrypted != NotesProvider.kUndecryptablePlaceholder) {
            final plain = RichContentCodec.plainText(decrypted).trim();
            if (plain.isNotEmpty) buffer.writeln(plain);
          } else {
            buffer.writeln('🔒 ${AppTranslations.translate("encrypted")}');
          }
        ...
        case NoteType.itinerary:
          for (final item in note.itineraryItems) {
            ...
            if (item.arrivalTime.isNotEmpty) {
              buffer.writeln(
                  '   🕐 ${AppTranslations.translate("arrival_time")}: ${item.arrivalTime}');
            }
            if (item.departureTime.isNotEmpty) {
              buffer.writeln(
                  '   🕑 ${AppTranslations.translate("departure_time")}: ${item.departureTime}');
            }
        ...
        case NoteType.mealPlan:
          for (final day in note.mealPlanItems) {
        ...
        case NoteType.recipe:
          ...
          if (r != null) {
            if (r.prepTime.isNotEmpty) {
              buffer.writeln(
                  '⏱ ${AppTranslations.translate("prep_time")}: ${r.prepTime}');
            }
            if (r.cookTime.isNotEmpty) {
              buffer.writeln(
                  '🔥 ${AppTranslations.translate("cook_time")}: ${r.cookTime}');
            }
            if (r.servings.isNotEmpty) {
              buffer.writeln(
                  '👥 ${AppTranslations.translate("servings")}: ${r.servings}');
            }
            if (r.ingredientRows.any((row) => row.name.isNotEmpty)) {
              ...
              in r.ingredientRows.where((row) => row.name.isNotEmpty)) {
                final qty = [row.quantity, row.unit]
                    ...
                    .join(' ');
                buffer
                    .writeln('• ${qty.isNotEmpty ? "$qty  " : ""}${row.name}');
              }
            }
            if (r.steps.any((s) => s.text.isNotEmpty)) {
              buffer.writeln(
                  '\n${AppTranslations.translate("steps").toUpperCase()}');
              int stepNum = 1;
              for (final step in r.steps.where((s) => s.text.isNotEmpty)) {
                buffer.writeln('$stepNum. ${step.text}');
                stepNum++;
              }
            }
          }
      ...
      if (note.imageIds.isNotEmpty) {
        buffer.writeln();
        buffer.writeln(
          AppTranslations.translate('shared_note_images_note')
              .replaceAll('{count}', '${note.imageIds.length}'),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: _buildAppBar(context, prov, isDark, notes),
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context, NotesProvider prov,
      bool isDark, List<Note> allNotes) {
    final textColor = Neu.textPrimary(isDark);
    final subColor = Neu.textSecondary(isDark);
    ...
    if (_isSelecting) {
      return AppBar(
        ...
        leading: IconButton(
          ...
          tooltip: AppTranslations.translate('clear_selection'),
        ),
        title: Text(
          '${_selectedNoteIds.length} ${AppTranslations.translate("selected")}',
          style: TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.w700,
            color: textColor,
        ...
        actions: [
          ...
          NeuIconButton(
            isDark: isDark,
            icon: Icon(
              _selectedNoteIds.length == allNotes.length
                  ? Icons.deselect_rounded
                  : Icons.select_all_rounded,
              color: subColor,
            ),
            onTap: () {
              if (_selectedNoteIds.length == allNotes.length) {
                _clearSelection();
              } else {
                _selectAll(allNotes);
              }
          ...
          NeuIconButton(
            ...
            onTap: () => _shareSelected(context, prov),
          ),
          const SizedBox(width: 6),
          // Delete
          NeuIconButton(
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: context.tr('search_notes'),
    ...
    return CustomScrollView(
      slivers: [
        for (final cat in present) ...[
          ...
          // Only render notes for this category when the section is expanded
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate(
                  ...
                  childCount: notes.where((n) => n.category == cat).length,
                ),
              ),
            )
          else
  ...
  Widget _buildDismissibleCard(
    ...
    // Get decrypted content for this note
    final decryptedContent = prov.getDecryptedContent(n);
    ...
    final card = Stack(
      children: [
        NoteCard(
          ...
          onLongPress: _isSelecting ? null : () => _toggleSelection(n.id),
          onTap: _isSelecting
    ...
    return Padding(
      ...
      child: Dismissible(
        ...
        background: Container(
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF2A1A1A) : const Color(0xFFFFE8E8),
            borderRadius: BorderRadius.circular(20),
...
class _NeuAlertDialog extends StatelessWidget {
  final bool isDark;
  final String? message; // Custom message; defaults to single-note delete text

  const _NeuAlertDialog({required this.isDark, this.message});
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(context.tr('delete_note'),
                style: TextStyle(
                    fontSize: 17,
                    fontWeight: FontWeight.w700,
                    color: textColor)),
            const SizedBox(height: 8),
            Text(message ?? context.tr('delete_note_to_bin'),
                style: TextStyle(fontSize: 14, color: subColor)),
            const SizedBox(height: 20),
            Row(
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        if (count != null) ...[
          const Spacer(),
          _CountBadge(count: count!, isDark: isDark),
        ],
      ]),
    );
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Expandable Section Label — used for category-sort headers.
// Shows the category name, note count, and a chevron that rotates on expand.
// ─────────────────────────────────────────────────────────────────────────────

class _ExpandableSectionLabel extends StatelessWidget {
  final String text;
  final IconData icon;
  final bool isDark;
  final int count;
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            _CountBadge(count: count, isDark: isDark),
            const SizedBox(width: 8),
            // Animated chevron — points down when expanded, right when collapsed
            AnimatedRotation(
              ...
              child: Icon(
                ...
                size: 16,
                color: expanded ? accent : subColor,
              ),
            ),
          ],
        ),
...
// ─────────────────────────────────────────────────────────────────────────────
// Count Badge — small pill showing a note count
// ─────────────────────────────────────────────────────────────────────────────
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    _NeuTypeChip(note: note, isDark: isDark, accent: accent),
                    const SizedBox(width: 8),
                    Text(
                      DateFormat('MMM d, yyyy · HH:mm').format(note.updatedAt),
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  _buildPreviewBody(
                      context, isDark, accent, textPrimary, textSub),
                ],
              ),
            ),
...
// ── Drawing note ────────────────────────────────────────────────────────────
  Widget _previewDrawing(BuildContext context, bool isDark, Color accent,
      Color textPrimary, Color textSub) {
    final content = Provider.of<NotesProvider>(context, listen: false)
        .getDecryptedContent(note);
    if (content == NotesProvider.kUndecryptablePlaceholder) {
      return Padding(
    ...
    try {
      final strokes = strokesFromJson(content);
      if (strokes.isEmpty) {
        return Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.brush_rounded, size: 48, color: accent.withAlpha(80)),
              const SizedBox(height: 8),
              Text(AppTranslations.translate('drawing'),
      ...
      return Center(
        child: Text(AppTranslations.translate('drawing'),
            style: TextStyle(fontSize: 14, color: textSub)),
      );
    }
  }
...
// ── Normal note ─────────────────────────────────────────────────────────────
  Widget _previewNormal(
    ...
    if (plainText.isEmpty) {
      return Text(context.tr('no_content'),
    ...
    for (final line in lines) {
      ...
      widgets.add(RichText(
        text: TextSpan(
          children: spans.isEmpty ? [TextSpan(text: line, style: base)] : spans,
        ),
      ));
      offset += lineLen + 1;
    }
  ...
  // ── Checklist ───────────────────────────────────────────────────────────────
  Widget _previewChecklist(BuildContext context, bool isDark, Color accent,
      Color textPrimary, Color textSub) {
    if (note.checklistItems.isEmpty) {
      return Text(context.tr('no_items'),
          style: TextStyle(color: textSub, fontSize: 14));
    }
    final done = note.checklistItems.where((i) => i.checked).length;
    final total = note.checklistItems.length;
          ...
          child: Row(children: [
            ...
            Text(
              ...
              style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w600,
                  color: done == total && total > 0 ? accent : textSub),
            ),
          ]),
        ),
  ...
  // ── Itinerary ────────────────────────────────────────────────────────────────
  Widget _previewItinerary(BuildContext context, bool isDark, Color accent,
      Color textPrimary, Color textSub) {
    if (note.itineraryItems.isEmpty) {
      return Text(context.tr('no_destinations'),
          style: TextStyle(color: textSub, fontSize: 14));
    }
    return Column(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (item.arrivalTime.isNotEmpty ||
                      ...
                      const SizedBox(height: 3),
                      Row(children: [
                        Icon(Icons.schedule_outlined, size: 12, color: textSub),
                        const SizedBox(width: 5),
                        Text(
  ...
  // ── Meal Plan ────────────────────────────────────────────────────────────────
  Widget _previewMealPlan(BuildContext context, bool isDark, Color accent,
      Color textPrimary, Color textSub) {
    if (note.mealPlanItems.isEmpty) {
      return Text(context.tr('no_days_planned'),
          style: TextStyle(color: textSub, fontSize: 14));
    }
    return Column(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  const SizedBox(width: 6),
                  Text(
                    day.day.isNotEmpty ? day.day : context.tr('date'),
                    style: TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w700,
                        color: textPrimary),
  ...
  // ── Recipe ───────────────────────────────────────────────────────────────────
  Widget _previewRecipe(BuildContext context, bool isDark, Color accent,
      Color textPrimary, Color textSub) {
    final r = note.recipeData;
    if (r == null) {
      return Text(context.tr('no_recipe_data'),
          style: TextStyle(color: textSub, fontSize: 14));
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
            ...
            children: [
              if (r.prepTime.isNotEmpty)
                _RecipeChip(
                    ...
                    label: '\${context.tr("prep_time")}: \${r.prepTime}',
                    accent: accent),
              if (r.cookTime.isNotEmpty)
                _RecipeChip(
                    isDark: isDark,
                    icon: Icons.local_fire_department_outlined,
                    label: '\${context.tr("cook_time")}: \${r.cookTime}',
                    accent: accent),
              if (r.servings.isNotEmpty)
                _RecipeChip(
                    isDark: isDark,
                    icon: Icons.people_outline_rounded,
                    label: '\${context.tr("servings")}: \${r.servings}',
                    accent: accent),
            ],
          ),
        const SizedBox(height: 16),
        if (r.ingredientRows.any((row) => row.name.isNotEmpty)) ...[
          _PreviewSectionLabel(
              text: context.tr('ingredients').toUpperCase(),
        ...
        if (r.steps.any((s) => s.text.isNotEmpty)) ...[
          _PreviewSectionLabel(
              text: context.tr('steps').toUpperCase(),
...
// ─────────────────────────────────────────────────────────────────────────────
// Drawing thumbnail painter — renders stroke data scaled to fit any box
// ─────────────────────────────────────────────────────────────────────────────

class _StrokeThumbnailPainter extends CustomPainter {
  final List<Stroke> strokes;
  final bool isDark;

  const _StrokeThumbnailPainter({required this.strokes, required this.isDark});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final stroke in strokes) {
      ...
      final path = Path();
      path.moveTo(
          transform(stroke.points.first).dx, transform(stroke.points.first).dy);
      for (int i = 1; i < stroke.points.length; i++) {
        final p = transform(stroke.points[i]);
  ...
  @override
  bool shouldRepaint(_StrokeThumbnailPainter old) =>
      old.strokes != strokes || old.isDark != isDark;
}
