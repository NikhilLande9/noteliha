...
class NoteInNoteSearchBarState extends State<NoteInNoteSearchBar>
  ...
  void _runSearch() {
    ...
    for (final item in widget.itineraryItems) {
      ...
      bool matched = false;
      if (item.location.isNotEmpty && item.location.toLowerCase().contains(q)) {
  ...
  void _findAll(
      String haystack, String needle, String source, List<_Match> out) {
    int offset = 0;
    while (true) {
    ...
    if (m.source == 'content') {
      ...
      if (widget.onStructuredMatch != null) {
        ...
        if (source.startsWith('checklist:')) {
          ...
          itemIndex =
              widget.recipeData?.ingredientRows.indexWhere((r) => r.id == id) ??
                  -1;
  ...
  Widget build(BuildContext context) {
    ...
    final label = hasMatches
        ...
        : (_queryCtrl.text.isEmpty
            ? ''
            : AppTranslations.translate('no_results'));
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: AppTranslations.translate('search_in_note'),
...
List<InlineSpan> overlaySearchHighlights(
  ...
  while (i < len) {
    ...
    final hl = hlColor[i];
    int j = i + 1;
    while (j < len && styles[j] == base && hlColor[j] == hl) {
      j++;
    }
    result.add(TextSpan(
