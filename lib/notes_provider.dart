...
/// Distinguishes which direction a sync operation is running.
/// Used by the UI to show direction-specific icons and colours.
enum SyncDirection { none, uploading, downloading }
...
class NotesProvider extends ChangeNotifier {
  ...
  Future<void> _checkDriveManifestExists() async {
    ...
    _setStatus(AppTranslations.translate('sync_checking_drive'),
    ...
    try {
      ...
      _setStatus(AppTranslations.translate('sync_drive_timeout'),
          isError: true);
    } catch (e) {
      debugPrint('Drive check error: $e');
      _driveAccountState = DriveAccountState.unknown;
      _driveCheckFailed = true;
      _setStatus(AppTranslations.translate('sync_drive_error'), isError: true);
    } finally {
  ...
  Future<void> _doCheckDriveManifest() async {
    ...
    if (rootList.files?.isEmpty ?? true) {
      ...
      _setStatus(AppTranslations.translate('sync_no_backup'), persistent: true);
      return;
    }
    ...
    await _fetchAndReDeriveKey(api, rootId,
    ...
    if (manifestList.files?.isNotEmpty ?? false) {
      ...
      _persistSyncState();
      _driveAccountState = DriveAccountState.returningUser;
      _setStatus(AppTranslations.translate('sync_backup_found'),
          persistent: true);
    } else {
      _persistSyncState();
      _driveAccountState = DriveAccountState.newUser;
  ...
  Future<String?> _fetchAndSupplyDriveSalt(
    ...
    try {
      ...
      if (list.files?.isEmpty ?? true) {
        debugPrint(
            '[Salt] No salt.bin on Drive — first use or pre-salt backup.');
        return null;
      }
      ...
      debugPrint(
          '[Salt] Salt fetched from Drive and supplied to EncryptionService.');
      return saltBase64;
    } catch (e) {
  ...
  Future<void> _fetchAndReDeriveKey(drive.DriveApi api, String rootFolderId,
    ...
    try {
      ...
      if (knownDriveSaltBase64 != null) {
        ...
        if (fetched == null) {
          debugPrint(
              '[RederiveKey] No salt.bin on Drive — nothing to re-derive.');
          return;
        }
  ...
  Future<String?> _showPasswordMismatchDialog(
    ...
    Future<bool> verifyInput(String pwd, StateSetter setDialogState) async {
      ...
      if (!valid) {
        setDialogState(() {
          errorText = AppTranslations.translate('incorrect_password_retry');
          isVerifying = false;
        });
      }
      return valid;
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      AppTranslations.translate('original_password_desc'),
                        ...
                        Expanded(
                          child: OutlinedButton(
                            ...
                            style: OutlinedButton.styleFrom(
                              ...
                              shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(12)),
                              side: BorderSide(color: subtle.withAlpha(80)),
                            ),
  ...
  void _rebuildSearchIndex() {
    if (_notesBox == null || _searchIndexBox == null || !_encryptionReady) {
      return;
    }
  ...
  String getDecryptedContent(Note note) {
    if (!_encryptionReady) {
      debugPrint(
          '[NotesProvider] getDecryptedContent called before encryption ready');
      return '';
    }
    ...
    if (note.noteType == NoteType.normal || note.noteType == NoteType.drawing) {
      ...
      if (result == null) {
        debugPrint(
            '[NotesProvider] Could not decrypt note ${note.id} — key mismatch');
  ...
  Future<String> saveImage(File imageFile) async {
    ...
    // Store as encrypted base64 string so images are protected the same
    // way as note content. Raw bytes -> base64 -> AES-GCM encrypt.
    final rawBase64 = base64Encode(bytes);
    final encrypted = EncryptionService.instance.encrypt(rawBase64);
    await _imagesBox?.put(imageId, encrypted);
    // Pre-populate the cache so the first render is instant.
    _imageCache[imageId] = rawBase64;
    _syncState.markImageDirty(imageId);
    _persistSyncState();
    return imageId;
  }

  /// In-memory cache: imageId → plain base64 string.
  /// Avoids re-decrypting (and re-encoding for legacy images) on every rebuild.
  final Map<String, String> _imageCache = {};

  /// Returns a plain base64 string ready for [base64Decode] / [Image.memory],
  /// or null if the image is missing or cannot be decrypted.
  String? getImage(String imageId) {
    final cached = _imageCache[imageId];
    if (cached != null) return cached;

    final stored = _imagesBox?.get(imageId);
    if (stored == null) return null;

    String? result;

    // NEW format: "ENC1:..." -> decrypt to get the raw base64 string.
    if (stored.startsWith('ENC1:')) {
      result = EncryptionService.instance.safeDecrypt(stored);
    } else {
      // Legacy path: JSON array of ints -> Uint8List -> base64.
      // Keeps existing images working after this upgrade.
      try {
        result = base64Encode(_decodeImage(stored));
      } catch (e) {
        debugPrint('[getImage] Legacy decode failed for $imageId: $e');
      }
    }

    if (result != null) _imageCache[imageId] = result;
    return result;
  }
  ...
  void _setStatus(String? msg,
    ...
    if (msg != null && !isSyncing && !persistent) {
      _syncStatusTimer = Timer(const Duration(seconds: 4), () {
        ...
        _syncDirection = SyncDirection.none;
        notifyListeners();
      });
    }
  }
  ...
  Future<void> uploadNotes() async {
    if (_currentUser == null || _notesBox == null) {
      _setStatus(AppTranslations.translate('sync_login_required'),
          isError: true);
      return;
    }
    if (isSyncing) return;

    _syncDirection = SyncDirection.uploading;
    _setStatus(AppTranslations.translate('sync_uploading'), isSyncing: true);
    ...
    try {
      ...
      if (dirtyNoteIds.isEmpty && dirtyImageIds.isEmpty) {
        _setStatus(AppTranslations.translate('sync_nothing_to_upload'));
        return;
      }

      final totalUpload = dirtyNoteIds.length;
      int uploadedCount = 0;
      // Translate once — counter appended separately so translation and
      // progress are fully decoupled.
      final uploadLabel = AppTranslations.translate('sync_uploading');
      for (final noteId in dirtyNoteIds) {
        final note = _notesBox!.get(noteId) as Note?;
        if (note == null) continue;
        uploadedCount++;
        _setStatus(
          '$uploadLabel ($uploadedCount / $totalUpload)',
          isSyncing: true,
        );
      ...
      _setStatus(AppTranslations.translate('sync_upload_complete'));
      notifyListeners();
    } catch (e, st) {
      _setStatus('${AppTranslations.translate("sync_upload_failed")}: $e',
          isError: true);
  ...
  Future<void> _uploadImage(drive.DriveApi api, String imageId) async {
    ...
    // Images are now stored as encrypted strings (ENC1:...). Upload the raw
    // stored string as UTF-8 bytes so Drive holds the encrypted form.
    // Legacy JSON-array images are uploaded as-is and will be migrated on
    // next getImage() call (they decode via the legacy path transparently).
    ...
    for (int i = 0; i < imageIds.length; i += maxConcurrent) {
      ...
      await Future.wait(
        batch.map((imgId) async {
          ...
          // Upload the stored string (encrypted ENC1:... or legacy JSON)
          // as UTF-8 bytes — Drive holds the encrypted form.
          final bytes = utf8.encode(rawData);
          final media = drive.Media(Stream.value(bytes), bytes.length,
              contentType: 'text/plain');
  ...
  Future<void> downloadNotes() async {
    ...
    _syncDirection = SyncDirection.downloading;
    _setStatus(AppTranslations.translate('sync_checking_updates'),
        isSyncing: true);
    ...
    try {
      ...
      if (toDownload.isEmpty) {
        _setStatus(AppTranslations.translate('sync_already_up_to_date'));
        return;
      }

      final totalDownload = toDownload.length;
      int downloadedCount = 0;
      // Translate once and strip any {count} placeholder — the counter is
      // appended separately so translation and progress are fully decoupled.
      final downloadLabel = AppTranslations.translate('sync_downloading')
          .replaceAll('{count}', '')
          .replaceAll('  ', ' ')
          .trim();
      _setStatus('$downloadLabel (0 / $totalDownload)', isSyncing: true);
      for (final noteId in toDownload) {
        downloadedCount++;
        _setStatus(
          '$downloadLabel ($downloadedCount / $totalDownload)',
          isSyncing: true,
        );
      ...
      _syncState.accountEmail = _currentUser?.email;
      _persistSyncState();
      _setStatus(AppTranslations.translate('sync_download_complete'));
      notifyListeners();
    } catch (e, st) {
      _setStatus('${AppTranslations.translate("sync_download_failed")}: $e',
          isError: true);
  ...
  Future<void> _downloadMissingImages(
    ...
    for (final imageId in manifest.imageIds) {
      ...
      try {
        ...
        // Drive holds the stored string verbatim (encrypted ENC1:... or
        // legacy JSON). Decode bytes back to string and store directly.
        await _imagesBox?.put(imageId, utf8.decode(buffer));
  ...
  Future<int> importNotesJson(Uint8List bytes) async {
    ...
    for (final raw in rawNotes) {
      try {
        final note = Note.fromJson(Map<String, dynamic>.from(raw as Map));
